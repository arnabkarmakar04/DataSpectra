...
def visualize_missing_data(data):
    ...
    if not missing_df.empty:
        ...
        fig = px.bar(
            ...
            labels={'Missing Count': 'Number of Missing Values'}, color='Missing Percent', color_continuous_scale='Reds'
        )
...
def display_cleaning():
    ...
    with st.expander("Expand to Clean and Transform Data"):
        ...
        with undo_col:
            st.write("") 
            if st.button("↩️", key="undo_button_icon", help="Undo last change", width='stretch'):
                if st.session_state.data_history:
        ...
        with redo_col:
            st.write("")
            if st.button("↪️", key="redo_button_icon", help="Redo last change", width='stretch'):
